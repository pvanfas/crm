...
def check_notification(request):
    ...
    count = Notification.objects.filter(
        user=user, is_read=False, is_active=True, shop=current_shop
    ).count()
...
def notifications(request):
    ...
    instances = Notification.objects.filter(
        user=request.user, is_deleted=False, is_active=True, shop=current_shop
    )

    query = request.GET.get("q")
    if query:
        instances = instances.filter(
            Q(subject__name__icontains=query) | Q(product__name__icontains=query)
        )
...
def revoke_access(request, pk):
    ...
    instance = get_object_or_404(
        Staff.objects.filter(pk=pk, is_deleted=False, shop=current_shop)
    )
