;(function ($) {
    $(document).on('autocompleteLightInitialize', '[data-autocomplete-light-function=select2]', function() {
        ...
        $(this).on('select2:selecting', function (e) {
            ...
            var select = $(this);
            var forward = $(this).parents('.form-group').find('select').attr('data-autocomplete-light-forward');
            var element_id = "";
            var forwarded_value = "";

            if (forward !== "" || forward !== null){
                var element_id = "#id_"+ forward;
                forwarded_value = $(element_id).val();

            }

            forward_data = {}
            if (forwarded_value !== ""){
                forward_data[forward] = forwarded_value ;
            }
